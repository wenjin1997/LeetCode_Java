...
//        int[] nums = {1,2,3,4,7};
        int k = 4;
        N6017_Solution solution = new N6017_Solution();
        System.out.println(solution.minimalKSum(nums, 1991601));
//        System.out.println(solution.minimalKSum(nums, k));
    }
...
//        }
        for (int i = 0; i < nums.length - 1; i++) {
            if (nums[i] < nums[i + 1] - 1) {
                ...
                if (k > n) {
                    res += (nums[i] + 1) * n + n * (n - 1) / 2;
                    k = k - n;
                } else {
                    res += (nums[i] + 1) * k + k * (k - 1) / 2;
                    return res;
                }
            }
...
//            }
        ...
        if (k > 0) {
            res += k * (nums[nums.length - 1] + 1) + k * (k - 1) / 2;
        }
//        for (int i = 1; i <= k; i++) {
//            res += nums[nums.length - 1] + i;
//        }
        return res;
    }
    ...
    public long minimalKSum2(int[] nums, int k) {
        Arrays.sort(nums);
        BigDecimal k0 = new BigDecimal(k);
        BigDecimal res1 = new BigDecimal(0);
        long res = 0;
        if (nums[0] > 1) {
            if (k >= nums[0]) {
                int n = nums[0] - 1;
                res1 = res1.add(new BigDecimal(1 * n + n * (n - 1) / 2));
//                res += 1 * n + n * (n - 1) / 2;
                k = k - n;
            } else {
                res1 = res1.add(new BigDecimal(1 * k + k * (k - 1) / 2));
//                res += 1 * k + k * (k - 1) / 2;
                return res;
            }
        }
...
//        }
        for (int i = 0; i < nums.length - 1; i++) {
            if (nums[i] < nums[i + 1] - 1) {
                int n = nums[i + 1] - 1 - nums[i];
                if (k > n) {
                    res1 = res1.add(new BigDecimal((nums[i] + 1) * n + n * (n - 1) / 2));
//                    res += (nums[i] + 1) * n + n * (n - 1) / 2;
                    k = k - n;
                } else {
                    res1 = res1.add(new BigDecimal((nums[i] + 1) * k + k * (k - 1) / 2));
//                    res += (nums[i] + 1) * k + k * (k - 1) / 2;
                    return res;
                }
            }
